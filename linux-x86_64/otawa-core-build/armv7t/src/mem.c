...
uint16_t arm_mem_read16(arm_memory_t *memory, arm_address_t address) {
	...
	uint8_t *p;
	if (pte->info)
		pte->info->callback_fun(address, 2, &res, ARM_MEM_READ, pte->info->callback_data);
...
uint32_t arm_mem_read32(arm_memory_t *memory, arm_address_t address) {
	...
	uint8_t *p;
	if (pte->info)
		pte->info->callback_fun(address, 4, &res, ARM_MEM_READ, pte->info->callback_data);
	else
	{
		p = pte->storage + offset;
...
#	ifdef ARM_MEM_SPY
    	mem->spy_fun(mem, address, sizeof(res), arm_access_read, mem->spy_data);
#	endif
    return res;
}
...
uint64_t arm_mem_read64(arm_memory_t *memory, arm_address_t address) {
	...
	if (pte->info)
		pte->info->callback_fun(address, 8, &res, ARM_MEM_READ, pte->info->callback_data);
