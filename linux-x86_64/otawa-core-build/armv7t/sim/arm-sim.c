...
FILE* load_profiling_file(char* profiling_file_name, int inst_stat[])
{
    ...
    if(profile_id == NULL)
    {
        ...
        for(i = 0; i<ARM_INSTRUCTIONS_NB; i++)
...
void write_profiling_file(FILE* profile_id, int inst_stat[])
{
	...
	}tmp, entries[ARM_INSTRUCTIONS_NB];
	...
	for(i = 0; i<ARM_INSTRUCTIONS_NB; i++)
	{
		for(j = i+1; j<ARM_INSTRUCTIONS_NB; j++)
		{
    ...
    for(i = 0; i<ARM_INSTRUCTIONS_NB; i++)
    {
        fprintf(profile_id, "%s %d %d\n",
        	arm_get_string_ident(entries[i].id),	// instruction name
...
int main(int argc, char **argv) {
    ...
    copy_options_to_arm_env(arm_get_sys_env(platform), &options);
    ...
    if(profile)
    {
        ...
        if( profiling_file_path != NULL){
			profiling_file_name = (char*)alloca(sizeof(char)* (strlen(profiling_file_path) + strlen(ARM_PROC_NAME) + 10));
			...
			strcat(profiling_file_name, ARM_PROC_NAME);
		}else{
			profiling_file_name = (char*)alloca(sizeof(char)* strlen(ARM_PROC_NAME) + 10);
			strcpy(profiling_file_name, ARM_PROC_NAME);
		}
...
#	endif
    ...
    if(!verbose && !profile)
    {
			...
			if(fast_sim)
				inst_cnt += arm_run_and_count_inst(sim);
			else
			{
				while(addr_exit != state->ARM_PC_NAME)
				{
					arm_step(sim);
					inst_cnt++;
				}
	...
	else
	{
        ...
        while(!arm_is_sim_ended(sim))
		{
            ...
            if(verbose)
            {
                arm_disasm(buffer, inst);
                fprintf(stderr, "%08x: %s\n", arm_next_addr(sim),  buffer);
            }
            arm_free_inst(inst);
            arm_step(sim);
			inst_cnt++;
