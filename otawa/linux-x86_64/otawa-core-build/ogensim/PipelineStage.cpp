...
void LazyStageIQIQ::action() {
	TRACE10(elm::cout << "LazyStage->action(): \n";)
	TRACE10(elm::cout << "\tin_number_of_accepted_outs=" << in_number_of_accepted_outs.read() << "\n";)
	...
	TRACE10(elm::cout << "\tout_number_of_accepted_ins=" << accepted_ins << "\n";)
	TRACE10(elm::cout << "\tin_number_of_ins=" << in_number_of_ins.read() << "\n";)
	for (int i=0 ; (i<in_number_of_ins.read()) && (i<accepted_ins) ; i++) {
		TRACE10(elm::cout << "\thandling at " << in_instruction[i].read()->inst()->address() << "\n";)

		SimulatedInstruction * _inst = in_instruction[i].read();
	    // use to track where the instruction is
		_inst->incLocation();

		TRACEX(2,
			elm::cout
					<< "DECODE SI with " << _inst->currentAddress()
					<< " for next address " << _inst->nextAddress()
					<< io::endl;
		)

		leaving_instructions.addLast(_inst);
	}
	int outs = 0;
	...
	TRACE10(elm::cout << "\tout_number_of_outs=" << outs << "\n";)
}
