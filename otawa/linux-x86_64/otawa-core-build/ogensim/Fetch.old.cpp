...
void FetchStage::fetch() {
	...
	do  {
		next_inst = sim_state->driver->nextInstruction(*sim_state, next_inst); // implemented in main.cpp
		if (next_inst != NULL) {
			TRACE(elm::cout << "\tfetching at " << next_inst->address() << "\n";)
			std::cout << "[" << __FILE__ << ":" << __LINE__ << "] " << "fetching at " << std::hex << next_inst->address() << " (" << std::dec << next_inst->address() << ")\n";
...
//				next_inst = sim_state->driver->nextInstruction(*sim_state, next_inst);
		...
		{
			std::cout << "[" << __FILE__ << ":" << __LINE__ << "] " << "NULL instruction fetched, consider the end of the execution\n";
			sim_state->stop();
		}
