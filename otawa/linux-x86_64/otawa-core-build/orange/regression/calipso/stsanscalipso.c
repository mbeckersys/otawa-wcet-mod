...
{
	int i;
# 38 "originaux/stsanscalipso.c"
	*Sum = 0;
# 39 "originaux/stsanscalipso.c"
	for(i = 0; i < 1000; i++)
# 40 "originaux/stsanscalipso.c"
		*Sum += Array[i];
# 41 "originaux/stsanscalipso.c"
	*Mean = *Sum / 1000;
}

double Square(double x)
{
# 48 "originaux/stsanscalipso.c"
	return x * x;
}
...
void Calc_Var_Stddev(double Array[], double Mean, double *Var, double *Stddev)
{
	int i;
	double diffs;
# 58 "originaux/stsanscalipso.c"
	diffs = 0.0;
# 59 "originaux/stsanscalipso.c"
	for(i = 0; i < 1000; i++)
# 60 "originaux/stsanscalipso.c"
		diffs += Square(Array[i] - Mean);
# 61 "originaux/stsanscalipso.c"
	*Var = diffs / 1000;
# 62 "originaux/stsanscalipso.c"
	*Stddev = sqrt(*Var);
}
...
# 72 "originaux/stsanscalipso.c"
	numerator = 0.0;
# 73 "originaux/stsanscalipso.c"
	Aterm = ((Bterm = 0.0));
# 74 "originaux/stsanscalipso.c"
	for(i = 0; i < 1000; i++)
# 74 "originaux/stsanscalipso.c"
	{
# 75 "originaux/stsanscalipso.c"
		numerator += ((ArrayA[i] - MeanA)) * ((ArrayB[i] - MeanB));
# 76 "originaux/stsanscalipso.c"
		Aterm += Square(ArrayA[i] - MeanA);
# 77 "originaux/stsanscalipso.c"
		Bterm += Square(ArrayB[i] - MeanB);
	}
# 81 "originaux/stsanscalipso.c"
...
{
	register int i;
# 94 "originaux/stsanscalipso.c"
	for(i = 0; i < 1000; i++)
# 95 "originaux/stsanscalipso.c"
		Array[i] = i + RandomInteger() / 8095.0;
}

void RandomInteger()
{
# 104 "originaux/stsanscalipso.c"
	Seed = ((Seed * 133 + 81)) % 8095;
# 105 "originaux/stsanscalipso.c"
	return Seed;
}
...
int ttime()
{
	struct tms buffer;
	int utime;
# 119 "originaux/stsanscalipso.c"
	utime = buffer.tms_utime / 60.0 * 1000.0;
# 120 "originaux/stsanscalipso.c"
	return utime;
}
...
# 131 "originaux/stsanscalipso.c"
	InitSeed();
# 134 "originaux/stsanscalipso.c"
	StartTime = ttime();
# 136 "originaux/stsanscalipso.c"
	Initialize(ArrayA);
# 137 "originaux/stsanscalipso.c"
	Calc_Sum_Mean(ArrayA, &SumA, &MeanA);
# 138 "originaux/stsanscalipso.c"
	Calc_Var_Stddev(ArrayA, MeanA, &VarA, &StddevA);
# 140 "originaux/stsanscalipso.c"
	Initialize(ArrayB);
# 141 "originaux/stsanscalipso.c"
	Calc_Sum_Mean(ArrayB, &SumB, &MeanB);
# 142 "originaux/stsanscalipso.c"
	Calc_Var_Stddev(ArrayB, MeanB, &VarB, &StddevB);
# 147 "originaux/stsanscalipso.c"
	Calc_LinCorrCoef(ArrayA, ArrayB, MeanA, MeanB);
# 149 "originaux/stsanscalipso.c"
	StopTime = ttime();
# 150 "originaux/stsanscalipso.c"
