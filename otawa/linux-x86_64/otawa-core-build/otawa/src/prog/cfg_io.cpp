...
void CFGSaver::processBB(WorkSpace *ws, CFG *cfg, BasicBlock *bb) {
	...
	if(!bb->isEnd()) {
		...
		set(bb_elt, ID_ATT, _ << bb->number());
		if (bb->isCall()) {
			set(bb_elt, TYPE_ATT, _ << CALL_VAL);
		} else if (bb->isEntry()) {
			set(bb_elt, TYPE_ATT, _ << ENTRY_ATT);
		} else if (bb->isExit()) {
			set(bb_elt, TYPE_ATT, _ << EXIT_ATT);
		}

		for(PropList::Iter prop(bb); prop; prop++) {
			// TODO: all properties as sub-tags
			//set(bb_elt, prop->id()->name(), _ ); // prop->id()->print(out, prop));
		}
	}
