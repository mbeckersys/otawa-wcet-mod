...
static hard::Register fpscr("fpscr", hard::Register::BITS, 32);
static hard::MeltedBank misc("misc", &sr, &fpscr, (void *) 0);
static hard::PlainBank sfpr("sfpr", hard::Register::FLOAT, 32, "s%d", 32);
static hard::PlainBank dfpr("dfpr", hard::Register::FLOAT, 64, "d%d", 32);
static const hard::RegBank *banks_tab[] = { &gpr, &misc, &sfpr, &dfpr };
static genstruct::Table<const hard::RegBank *> banks_table(banks_tab, 4);
...
#define _DFPR(i)		dfpr[i]->platformNumber()
#define _SFPR(i)		sfpr[i]->platformNumber()
#define _CPSR()			sr.platformNumber()
#define _FPSR()			fpsr.platformNumber()
