 ...
 */
void add_to_list(list_entry_t **m, const char *n, ppc_address_t a) {

#	ifdef PPC_PROCESS_CODE_LABEL
		{ PPC_PROCESS_CODE_LABEL(a); }
#	endif
...
*/
int get_label_from_list(list_entry_t *m, ppc_address_t addr, const char **name) {
...
*/
list_entry_t *get_closer_label_from_list(list_entry_t *m, ppc_address_t addr) {
...
static void fail_with_help(const char *msg, ...) {
	...
	if(ppc_modes[0].name) {
		...
		for(i = 0; ppc_modes[i].name; i++) {
			if(!fst)
				fprintf(stderr, ", ");
			else
				fst = 0;
			fputs(ppc_modes[i].name, stderr);
		}
		fprintf(stderr, "\n");
	}
 ...
 */
char *ppc_solve_label_disasm(ppc_address_t address) {
	static char buf[256];
...
int main(int argc, char **argv) {
	ppc_platform_t *pf;
	int s_it;
	ppc_loader_sect_t *s_tab;
	int sym_it;
	int nb_sect_disasm = 0;
	ppc_loader_t *loader;
	int max_size = 0, i, j;
	char *exe_path = 0;
	ppc_inst_t *(*decode)(ppc_decoder_t *decoder, ppc_address_t address) = ppc_decode;
	int i_sect;
	ppc_decoder_t *d;

	/* test arguments */
	for(i = 1; i < argc; i++) {
		if(ppc_modes[0].name && strcmp(argv[i], "-m") == 0) {
			i++;
			if(i >= argc)
				fail_with_help("no argument for -m option");
			for(j = 0; ppc_modes[j].name; j++)
				if(strcmp(argv[i], ppc_modes[j].name) == 0) {
					decode = ppc_modes[j].decode;
					break;
				}
			if(!ppc_modes[j].name)
	...
	loader = ppc_loader_open(exe_path);
	if (loader == NULL) {
	...
	/* display sections */
	printf("found %d sections in the executable %s\n", ppc_loader_count_sects(loader)-1, exe_path);
	s_tab = (ppc_loader_sect_t *)malloc(ppc_loader_count_sects(loader) * sizeof(ppc_loader_sect_t));
	for(s_it = 0; s_it < ppc_loader_count_sects(loader); s_it++) {
		ppc_loader_sect_t data;
		ppc_loader_sect(loader, s_it, &data);
		if(data.type == PPC_LOADER_SECT_TEXT) {
	...
	/* display symbols */
	printf("\nfound %d symbols in the executable %s\n", ppc_loader_count_syms(loader)-1, exe_path);
	for(sym_it = 0; sym_it < ppc_loader_count_syms(loader); sym_it++) {
		ppc_loader_sym_t data;
		ppc_loader_sym(loader, sym_it, &data);
		if(data.type == PPC_LOADER_SYM_CODE || data.type == PPC_LOADER_SYM_DATA) {
			printf("[L]");
	...
	ppc_solve_label = ppc_solve_label_disasm;
	...
	/* load it */
	ppc_loader_load(loader, pf);

	d = ppc_new_decoder(pf);
	/* multi iss part, TODO: improve */
	ppc_state_t *state = ppc_new_state(pf);
	...
	ppc_set_cond_state(d, state);

	/* compute instruction max size */
	for(i = 1; i < PPC_TOP; i++) {
		int size = ppc_get_inst_size_from_id(i) / 8;
		if(size > max_size)
			max_size = size;
	}
	...
	for(i_sect = 0; i_sect<nb_sect_disasm; i_sect++) {
		...
		/* display new section */
		ppc_address_t adr_start = s_tab[i_sect].addr;
		ppc_address_t adr_end = s_tab[i_sect].addr + s_tab[i_sect].size;
		ppc_address_t prev_addr = 0;
		...
		while (adr_start < adr_end) {
			...
			int size;
			char buff[100];
			ppc_inst_t *inst = decode(d, adr_start);
			...
			size = ppc_get_inst_size(inst) / 8;
			for(i = 0; i < max_size; i++) {
				if(i < size)
					printf("%02x", ppc_mem_read8(ppc_get_memory(pf, 0), adr_start + i));
				else
					fputs("  ", stdout);
			}
